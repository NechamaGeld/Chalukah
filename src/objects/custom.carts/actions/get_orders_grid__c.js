...
$http.get("crud/custom.carts/count")
    .then(({ data }) => {
        scope.count= data?.[0]?.count
    })

$http.get("crud/custom.carts/read?$clms=id&$order_by=id&$order=desc")
    .then(({ data }) => {
...
function buildGrid(data) {
    ...
    scope.gridOptions = {
        ...
        columnDefs: [
            ...
            { name: 'price_range__c', displayName: 'Price Range' },
...
function processCartRow(row) {
    return {
        ...
        price_range__c: row.price_range__c,
